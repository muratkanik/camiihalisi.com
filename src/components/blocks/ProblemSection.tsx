...
export default async function ProblemSection({
  ...
  const DEFAULT_ITEMS: ProblemItem[] = [
    { title: t("quality"),       desc: t("qualityDesc"),       iconIndex: 0 },
    { title: t("customSize"),    desc: t("customSizeDesc"),    iconIndex: 1 },
    { title: t("fastProduction"),desc: t("fastProductionDesc"),iconIndex: 2 },
  ];

  const displayTitle = sectionTitle ?? t("mainTitle");
      ...
      <div className="container-site">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {cards.map((p, i) => (
            <div
              key={i}
              className="bg-[#0097A7] rounded-2xl p-8 flex flex-col gap-5 hover:bg-[#00494D] transition-colors"
            >
