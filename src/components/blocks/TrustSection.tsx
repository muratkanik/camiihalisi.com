...
const DEFAULT_TESTIMONIALS: Testimonial[] = [
  {
    ...
    quote: "Doğru halıyı seçmek sayesinde yıllardır sorunsuz bir şekilde kullanıyoruz. Destek ve profesyonelliklerinden dolayı Allah razı olsun.",
    rating: 5,
  },
  {
    name: "Hasan Efendi",
    location: "İstanbul — Camii Vakfı",
    quote: "10 yıl önce döşettik, hâlâ ilk günkü gibi duruyor. Rengi solmadı, deformasyona uğramadı. Asil Halı'yı herkese tavsiye ederiz.",
    rating: 5,
  },
];
...
export default async function TrustSection({ testimonials }: TrustSectionProps) {
            ...
            {items.slice(0, 3).map((item, i) => (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-bold text-sm text-[#0D1B1E]">{item.name}</div>
                    <div className="text-xs text-[#5A6A6D]">{item.location}</div>
                  </div>
