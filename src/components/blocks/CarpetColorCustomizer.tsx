...
export default function CarpetColorCustomizer({ isOpen, onClose, imageSrc, patternName, locale }: CarpetColorCustomizerProps) {
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      // Small delay to allow UI to show "Analyzing" state
      setTimeout(() => {
  ...
  const handleDownload = () => {
    ...
    link.download = `asil_hali_${patternName.replace(/\s+/g, '_')}_custom.jpg`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex bg-white/95 dark:bg-slate-950/95 backdrop-blur-md">
      {/* HEADER BAR */}
      <div className="absolute top-0 left-0 right-0 h-16 border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 flex items-center justify-between px-6 z-10">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={handleDownload} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[#003B40] text-white text-sm font-medium hover:bg-[#002a2e] transition-colors shadow-lg">
      ...
      <div className="flex w-full h-full pt-16">
        ...
        <div className="flex-1 flex flex-col h-full bg-slate-50 dark:bg-slate-950/50">
          ...
          <div className="h-40 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 p-4">
                ...
                >
                  ...
                  <div className="p-2 bg-slate-50 dark:bg-slate-800 text-xs text-center relative group">
                    ...
                    <div className="absolute inset-0 bg-slate-50 dark:bg-slate-800 flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button 
                        onClick={(e) => { e.stopPropagation(); toggleProtect(color.id); }}
                        className="p-1 rounded hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300"
                        title={color.isProtected ? "Kilidi Aç (Değiştirilebilir)" : "Kilitle (Korunan Renk)"}
                      >
                        {color.isProtected ? <Unlock className="w-4 h-4"/> : <Lock className="w-4 h-4"/>}
                      </button>
                      {color.mappedYarn && (
                        <button 
                          onClick={(e) => { e.stopPropagation(); removeMapping(color.id); }}
                  ...
                  {/* Indicator if mapped */}
                  {color.mappedYarn && (
                    <div className="h-1.5 w-full bg-[#C9972B]" />
                  )}
                </div>
              ))}
            </div>
        ...
        <div className="w-96 bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-800 flex flex-col h-full">
