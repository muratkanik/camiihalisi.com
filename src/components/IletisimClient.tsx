      ...
      <section className="bg-[#006064] py-20 relative overflow-hidden">
        ...
        <div className="container-site relative z-10">
          <nav className="flex items-center gap-2 text-sm text-white/50 mb-6">
            <Link href={`${prefix}/`} className="hover:text-white transition-colors">{nav("home")}</Link>
        ...
        <div className="container-site">
          <div className="grid lg:grid-cols-2 gap-12 mb-16">
                ...
                <>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <button type="submit" disabled={loading} className="btn btn-primary w-full justify-center">
                      {loading ? t("sending") : t("send")} <Send className="w-4 h-4" />
                    </button>
                  </form>
                </>
              )}
            ...
            <div className="space-y-4">
              <div>
                <span className="badge badge-gold mb-4">{t("directContact")}</span>
                <h2 className="section-title mb-2">{t("reachUs")}</h2>
              ...
              <a href={`/api/r?to=https%3A%2F%2Fwa.me%2F${settings.whatsappNumber}%3Ftext%3D${settings.whatsappMessage}&from=%2Filetisim&label=whatsapp-card&cat=whatsapp`} target="_blank" rel="noopener"
                ...
                <div className="flex-1">
                  <div className="font-bold text-sm">{t("whatsappBtn")}</div>
                  <div className="text-white/80 text-sm">{t("whatsappSub")}</div>
                </div>
              ...
              <div className="bg-[#C9972B] rounded-2xl p-5 text-[#1A1A1A]">
                <p className="text-sm font-semibold mb-1">{t("officialSite")}</p>
                ...
                <p className="text-xs mt-1 opacity-70">{t("officialSiteSub")}</p>
              </div>
            </div>
          </div>
          ...
          {activeOffices.length > 0 && (
            <div>
              <div className="text-center mb-10">
                <span className="badge badge-gold mb-4">{t("locationsBadge")}</span>
                <h2 className="section-title">{t("locationsTitle")}</h2>
                ...
                {Object.entries(byRegion).map(([region, regionOffices]) => (
                  <div key={region} className="rounded-2xl border border-[#B2EBF2] overflow-hidden shadow-sm">
                    ...
                    <div className="bg-[#006064] px-6 py-4 flex items-center gap-3">
                      ...
                      <h3 className="text-base font-bold text-white tracking-wide">{region}</h3>
                      <span className="ml-auto text-xs font-medium text-white/50 bg-white/10 px-2.5 py-1 rounded-full">
                        {regionOffices.length} {t("locationCount")}
                      </span>
                    </div>
                    {/* Offices grid */}
                    <div className="bg-white p-5">
                      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {regionOffices.map((office) => {
                          ...
                          return (
                            <div key={office.id} className="bg-[#F0FDFE] rounded-xl border border-[#B2EBF2] hover:border-[#C9972B]/40 hover:shadow-sm transition-all p-4">
                              <div className="flex items-start gap-3 mb-3">
                                <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${TYPE_COLOR[office.type] ?? "bg-slate-600 text-white"}`}>
                                  <TypeIcon className="w-4 h-4" />
                                </div>
                                <div>
                                  <div className="font-bold text-sm text-[#1A1A1A] leading-tight">{office.title}</div>
                                  <span className="text-[10px] bg-white text-[#6B6355] border border-[#B2EBF2] px-1.5 py-0.5 rounded font-medium">
                              ...
                              {office.whatsapp && (
                                <a href={`https://wa.me/${office.whatsapp.replace(/[^0-9]/g, "")}`} target="_blank" rel="noopener"
                                  ...
                                  <span className="font-medium">{t("whatsappWrite")}</span>
                                </a>
                              )}
                              {office.workHours && (
