import { getHeroSlides, addHeroSlideAction, removeHeroSlideAction, toggleHeroSlideAction, initDefaultSlidesAction } from "./actions";
import { Image as ImageIcon, Plus, Eye, EyeOff, Trash2, RefreshCw } from "lucide-react";
  ...
  return (
    <div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-6 mb-8">
        ...
        <form action={addHeroSlideAction} className="flex flex-col sm:flex-row gap-3">
          <input
            name="imageUrl"
            type="text"
            required
            placeholder="Görsel URL'si — örn: /images/hd-foto-21.jpg"
            className="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-sm text-slate-800 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#C9972B]/40 focus:border-[#C9972B]"
          />
          <input
            name="alt"
            type="text"
            placeholder="Alt metin (isteğe bağlı)"
            className="w-48 px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-sm text-slate-800 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#C9972B]/40 focus:border-[#C9972B]"
          />
          <button
            type="submit"
            className="px-6 py-2.5 rounded-xl bg-[#C9972B] text-white font-bold text-sm hover:bg-[#B8861F] transition-all flex items-center gap-2"
          >
            <Plus className="w-4 h-4" />
            Ekle
          </button>
