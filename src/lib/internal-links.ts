...
export function applyLinks(text: string, links: LinkMap): string {
  ...
  for (const kw of keywords) {
    ...
    result = result.replace(regex, (match) => {
      return `<a href="${url}" class="text-[#0097A7] underline underline-offset-2 decoration-[#C9972B]/60 hover:decoration-[#C9972B] font-medium transition-colors" title="${kw}">${match}</a>`;
    });
  }
